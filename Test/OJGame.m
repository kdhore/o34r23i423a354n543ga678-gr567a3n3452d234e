classdef OJGame
    ...
    methods
        ...
        function obj = updatefromSim(obj,decisions, proc_plants, storage)
            ...
            plants_open = find(obj.proc_plant_cap);
            for i=1:length(plants_open)
                obj.proc_plant_inv(plants_open(i)).ORA = proc_plants{plants_open(i)}.ora;
            end
            obj.proc_plant_cap = obj.proc_plant_cap + decisions.proc_plant_dec; 
            storage_open = find(obj.storage_cap);
            for i=1:length(storage_open)
                index = storage_open(i);
                obj.storage_inv(index).ORA = storage{i}.inventory{1};
                obj.storage_inv(index).POJ = storage{i}.inventory{2};
                obj.storage_inv(index).ROJ = storage{i}.inventory{3};
                obj.storage_inv(index).FCOJ = storage{i}.inventory{4};
            end
            obj.storage_cap = obj.storage_cap + decisions.storage_dec;
            obj.futures_years = obj.futures_years + 1;
