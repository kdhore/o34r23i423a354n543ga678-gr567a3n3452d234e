...
% Will return the demand for each city
    ...
    ORA_params = load('ORA_params.mat');
    POJ_params = load('POJ_params.mat');
    ROJ_params = load('ROJ_params.mat');
    FCOJ_params = load('FCOJ_params.mat');

    ORA_params = ORA_params.ORA_params;
    POJ_params = POJ_params.POJ_params;
    ROJ_params = ROJ_params.ROJ_params;
    FCOJ_params = FCOJ_params.FCOJ_params;
    ...
    for i = 1:numel(cities)/3
       ...
       ora = ora + normrnd(ORA_params{region_ind,1},ORA_params{region_ind,2});
       ...
       poj = poj + normrnd(POJ_params{region_ind,1},POJ_params{region_ind,2});
       ...
       fcoj = fcoj + normrnd(FCOJ_params{region_ind,1},FCOJ_params{region_ind,2});
       ...
       roj = roj + normrnd(ROJ_params{region_ind,1},ROJ_params{region_ind,2});
