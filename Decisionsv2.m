    ...
    methods
        ...
        function Decision = Decisions(filename,OJ_object,pricesORA,...
            ...
            if nargin > 0
                ...
                Decision.pricing_ORA_dec = yr2014.pricing_ORA_dec - 0.7;
                Decision.pricing_POJ_dec = yr2014.pricing_POJ_dec - 0.4;
                Decision.pricing_ROJ_dec = yr2014.pricing_ROJ_dec - 0.7;
                Decision.pricing_FCOJ_dec = yr2014.pricing_FCOJ_dec - 0.8;
...
%                     struct('POJ', nan, 'FCOJ', nan);
                ...
                for i = 1:length(plants_open)
                    CumDemandoverProcPlantORA = CumDemandoverProcPlantORA + ...
                        sum(Decision.demandProcPlantPOJ(i, :))+ sum(Decision.demandProcPlantFCOJ(i, :));
                end
                
                for i = 1:6
                    for j = 1:length(plants_open)
                        Decision.ship_grove_dec(i, j)...
                              =(sum(Decision.demandProcPlantPOJ(j, :))+...
                             sum(Decision.demandProcPlantFCOJ(j, :)))/...
                             CumDemandoverProcPlantORA;

%                             =min((sum(Decision.demandProcPlantPOJ(j, :))+...
%                             sum(Decision.demandProcPlantFCOJ(j, :))),OJ_object.proc_plant_cap(plants_open(j),1))/...
%                             CumDemandoverProcPlantORA;
