classdef YearDataWShipping
    ...
    methods
        ...
        function yr = YearDataWShipping(filename,OJ_object)
            if nargin > 0
                ...
                yr.shippingSchedule = cell(10,6);
                yr.tankersAvailable = zeros(length(non_zero),1); % non_zero = processing plants open
                for i = 1:71
                    storage_schedule{i} = schedule;
                end
                for j = 1:10
                    for i = 1:6
                        yr.shippingSchedule{j,i} = storage_schedule;
                    end
                end
                
                storage_open = find(OJ_object.storage_cap);
                ship_temp = cell(length(non_zero),1);
                for i = 1:length(non_zero)
                    ship_temp{i} = cell(length(storage_open),1);
                    for j = 1:length(storage_open)
                        ship_temp{i}{j} = zeros(6,4);
                    end
                end
                for i = 1:length(non_zero)
                    shtname = char(plantNamesInUse(non_zero(i)));
                    for j = 1:length(storage_open)
                        start_range = 36 + 11*(j-1);
                        [~,~,ship_temp_data] = xlsread(filename,shtname,strcat('AV',num2str(start_range),':','AY',num2str(start_range+5)));
                        ship_temp{i}{j} = cell2mat(cellNaNReplace(ship_temp_data,0));
                        % four weeks ago
                            if (ship_temp{i}{j}(6,1) == 4)
                               yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.POJ_4Week = ship_temp{i}{j}(4,1);
                               yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.FCOJ_4Week = ship_temp{i}{j}(5,1);
                            end
                    
                            % three weeks ago
                            if (ship_temp{i}{j}(6,2) == 4)
                                yr.shippingSchedule{non_zero(i),4}{storage_open(j)}.POJ_4Week = ship_temp{i}{j}(4,2);
                                yr.shippingSchedule{non_zero(i),4}{storage_open(j)}.FCOJ_4Week = ship_temp{i}{j}(5,2);
                            elseif (ship_temp{i}{j}(6,2) == 3)
                                yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.POJ_3Week = ship_temp{i}{j}(4,2);
                                yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.FCOJ_3Week = ship_temp{i}{j}(5,2);
                            end
                            
                            % two weeks ago
                            if (ship_temp{i}{j}(6,3) == 4)
                                yr.shippingSchedule{non_zero(i),5}{storage_open(j)}.POJ_4Week = ship_temp{i}{j}(4,3);
                                yr.shippingSchedule{non_zero(i),5}{storage_open(j)}.FCOJ_4Week = ship_temp{i}{j}(5,3);
                            elseif (ship_temp{i}{j}(6,3) == 3)
                                yr.shippingSchedule{non_zero(i),4}{storage_open(j)}.POJ_3Week = ship_temp{i}{j}(4,3);
                                yr.shippingSchedule{non_zero(i),4}{storage_open(j)}.FCOJ_3Week = ship_temp{i}{j}(5,3);
                            elseif (ship_temp{i}{j}(6,3) == 2)
                                yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.POJ_2Week = ship_temp{i}{j}(4,3);
                                yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.FCOJ_2Week = ship_temp{i}{j}(5,3);
                            end
                            
                            % for the last week
                            if (ship_temp{i}{j}(6,4) == 4)
                                yr.shippingSchedule{non_zero(i),6}{storage_open(j)}.POJ_4Week = ship_temp{i}{j}(4,4);
                                yr.shippingSchedule{non_zero(i),6}{storage_open(j)}.FCOJ_4Week = ship_temp{i}{j}(5,4);
                            elseif (ship_temp{i}{j}(6,4) == 3)
                                yr.shippingSchedule{non_zero(i),5}{storage_open(j)}.POJ_3Week = ship_temp{i}{j}(4,4);
                                yr.shippingSchedule{non_zero(i),5}{storage_open(j)}.FCOJ_3Week = ship_temp{i}{j}(5,4);
                            elseif (ship_temp{i}{j}(6,4) == 2)
                                yr.shippingSchedule{non_zero(i),4}{storage_open(j)}.POJ_2Week = ship_temp{i}{j}(4,4);
                                yr.shippingSchedule{non_zero(i),4}{storage_open(j)}.FCOJ_2Week = ship_temp{i}{j}(5,4);
                            elseif (ship_temp{i}{j}(6,4) == 1)
                                yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.POJ_1Week = ship_temp{i}{j}(4,4);
                                yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.FCOJ_1Week = ship_temp{i}{j}(5,4);
                            end
                            % get amount by tankers
                            yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.POJ_1Week = ...
                                yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.POJ_1Week + ship_temp{i}{j}(1,4);
                            yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.FCOJ_1Week = ...
                                yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.FCOJ_1Week + ship_temp{i}{j}(2,4);
                            yr.shippingSchedule{non_zero(i),3}{storage_open(j)}.Tankers = ship_temp{i}{j}(3,4);
                            
                            % get tankers coming home
                            yr.shippingSchedule{non_zero(i),2}{storage_open(j)}.Tankers = ship_temp{i}{j}(3,3);
                    end
                    % tankers available is a vector for each processing
                    % plant that is open
                    tankers_avail = xlsread(filename,shtname,'AY27');
                    yr.tankersAvailable(i) = tankers_avail;
                end
